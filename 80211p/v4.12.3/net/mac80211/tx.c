...
//	if(local->hw.wiphy->dot11OCBActivated == 0) {
	/* don't support encryption in 802.11p */
					   ...
					   struct sta_info *sta)
{
	...
	/* convert 
Ethernet header to proper 802.11 header (based on
	 * operation mode) */
	/* TODO: add ethertype for PF_WSMP? */
...
void ieee80211_check_fast_xmit(struct sta_info *sta)
{
	...
	if (sdata->vif.type == NL80211_IFTYPE_OCB) {
		printk("%s:%s ocb doesnt support fast xm\n",__FILE__,__FUNCTION__);
		return;
	}
	...
	printk("%s:%s checking fast xmit building chan defs\n",__FILE__,__FUNCTION__);
	 ...
	 */
	printk("%s:%s checking fast xmit skipping ciphers\n",__FILE__,__FUNCTION__);
...
if(sdata->vif.type != NL80211_IFTYPE_OCB) {
	...
	hdr->frame_control = fc;
	printk("%s:%s checking fast xmit building hdrs\n",__FILE__,__FUNCTION__);

	memcpy(build.hdr + build.hdr_len,
	       rfc1042_header,  sizeof(rfc1042_header));
	build.hdr_len += sizeof(rfc1042_header);
	printk("%s:%s checking fast xmit testing fast tx\n",__FILE__,__FUNCTION__);
 ...
 out:
 	printk("%s:%s checking fast xmit in OUT section\n",__FILE__,__FUNCTION__);
